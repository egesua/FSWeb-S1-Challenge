...
function indekseGoreFenomen(fenomenler, index) {
  let fenomen = fenomenler[index];
  let metin = index + ". indekste bulunan fenomen: " + fenomen.profile;
  return metin;
}
...
function fenomenEkle(dizi, sirano, profiladi, takipçi, gonderi, platform) {
  const yeniDizi = [...dizi];
  const yeniFenomen = {
    number: sirano,
    profile: profiladi,
...
function enFenomenler(fenomenler) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000) {
      yeniFenomenler.push(fenomenler[i].profile);
    }
  }
